...
class MyBertForTokenClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(config.hidden_size, 512)
        self.classifier2 = nn.Linear(512, config.num_labels)
    ...
    def forward(
        ...
        logits = self.classifier2(sequence_output)
...
def train(train_iter, eval_iter, tag2idx, config, bert_model="bert-base-uncased"):
    ...
    model = MyBertForTokenClassification.from_pretrained(bert_model, num_labels=len(tag2idx))
    model.train()
...
def load_model(config, do_lower_case=True):
    ...
    model = MyBertForTokenClassification.from_pretrained(config.bert_model, num_labels=len(tag2idx))
